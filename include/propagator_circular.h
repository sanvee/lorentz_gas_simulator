...
public:
    ...
    CircularPropagator ( Simbox &simbox, Trajectory::Circular &trajectory, Neighbourlist::Circular &neighlist )
    {
        ...
        {
            std::cout << std::endl << "---Propagator(circular_non_directional)---" << std::endl;
...
public:
        ...
        while ( trajectory_->current_time_ < end_time )
        {
            ...
            if ( canLeaveContainer ( *container_, *trajectory_ ) )
            {
                ...
                if ( nr_reenter == 0 )
                {
                    ...
                    n_neight = neighlist_->getNeighbours ( trajectory_->cx_begin_, trajectory_->cy_begin_, cont_inters_[2*i_low_c], cont_inters_[2*i_low_c + 1], trajectory_->rx_, trajectory_->ry_ );
                }
                else
                {
                    ...
                    n_neight = neighlist_->getNeighbours ( cont_inters_[2*i_low_c], cont_inters_[2*i_low_c+1],cont_inters_[2*i_low2_c], cont_inters_[2*i_low2_c+1],  trajectory_->rx_, trajectory_->ry_ );
                }
            }
            else
            {
                ...
                n_neight = neighlist_->getNeighbours ( trajectory_->cx_begin_, trajectory_->cy_begin_, trajectory_->rx_, trajectory_->ry_ );
            }
    ...
    inline int getIntersections ( Obstacles::Circles &obstacles, int index_last )
    {
        double x_tr = trajectory_->rx_;
        double y_tr = trajectory_->ry_;
        double r_tr = trajectory_->radius_;
        ...
        {
            n_obs = neighlist_->neighbour_obstacles_[i];

            x_obs = obstacles.x_[n_obs];
            y_obs = obstacles.y_[n_obs];
            r_obs = obstacles.radius_[n_obs];

            max_dist_sq = ( r_obs + r_tr ) * ( r_obs + r_tr );
            min_dist_sq = ( r_obs - r_tr ) * ( r_obs - r_tr );
...
//Traj: Circle; Obstacle: Squares
    inline int getIntersections ( Obstacles::Squares &obstacles, int index_last )
    {
        double x_tr = trajectory_->rx_;
        double y_tr = trajectory_->ry_;
        double r_tr = trajectory_->radius_;
...
//Traj: Circle; Obstacle: Crosses
    inline int getIntersections ( Obstacles::Crosses &obstacles, int index_last )
    {
        double x_tr = trajectory_->rx_;
        double y_tr = trajectory_->ry_;
        double r_tr = trajectory_->radius_;
