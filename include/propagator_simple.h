...
public:
    ...
    SimplePropagator ( Simbox &simbox, Trajectory::Circular &trajectory, Neighbourlist::Simple&neighlist, bool filter )
    {
    ...
    template <class T_Simbox>
    inline void setStartPositions ( T_Simbox &simbox )
    {
        ...
        for ( auto i = 0; i < trajectory_->n_trajectories_; ++i )
        {
            do
            {
                x = uni_dist ( Globalrand::e2 ) * simbox.container_->lx_;
                y = uni_dist ( Globalrand::e2 ) * simbox.container_->ly_;

                index_end = neighlist_->getPossibleCoveringNeighbours ( x, y );
            }
            while ( simbox.obstacles_->isInside ( neighlist_->neighbour_obstacles_, index_end, x, y ) );
            ...
            if ( filter_circling_ )
            {
                ...
                if ( !checkIntersections ( *simbox.obstacles_ ) )
                {
    ...
    template <class T_Simbox>
    inline void setStartPositions_no_check ( T_Simbox &simbox )
    {
        std::uniform_real_distribution<double> uni_dist ( 0, 1 );
...
public:
    ...
    /*
        //Version A (bad) using the big radius as reference
        //Traj: Circle; Obstacle: Circle
        inline int getIntersections ( Obstacles::Circles &obstacles, int index_last )
        {
            double x_tr = trajectory_->rx_;
            ...
            for ( auto i = 0; i < index_last; i++ )
            {
                ...
                //compute the distance from the center of the trajectory circle on the jonction line of the two centers
                u = d_sq + ( r_tr*r_tr - r_obs*r_obs );
                //u = d_sq + ( r_tr - r_obs ) * ( r_tr + r_obs );
                u /= 2.0 * d;

                //compute the vertical offset from this line
                v = std::sqrt (r_tr * r_tr - u * u);
